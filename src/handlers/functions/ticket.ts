...
import { defaultPFP, DummyUser, LanguageAliases } from '../../assets/utils/static';
...
import { htmlTemplate } from '../../config/template';

module.exports = async (client: any) => {

    client.getTicketData = async function (interaction: Interaction) {
    ...
    client.getChannelTicket = async function (interaction: Interaction) {
        const ticketChannelData = await ticketChannelsModel.findOne({ Guild: interaction.guild?.id, channelID: interaction.channel?.id })
        return ticketChannelData;
    }

    client.isTicket = async function (interaction: Interaction) {
        const ticketChannelData = await ticketChannelsModel.findOne({ Guild: interaction.guild?.id, channelID: interaction.channel?.id });
    ...
    client.transcript = async function (interaction: any, channel: any) {
        const template = htmlTemplate;
        const messages: any[] = await interaction.channel.messages.fetch({ limit: 100 });
        ...
        document.getElementsByClassName('preamble__guild-icon')[0].setAttribute('src', interaction.channel.guild.iconURL());
        // @ts-ignore
        ...
        for (const message of (Array.from(messages.values())).sort((a: any, b: any) => a.createdTimestamp - b.createdTimestamp)) {
            ...
            if (message.reference?.messageId) {
                ...
                const author = referencedMessage?.author ?? DummyUser;

                reference.innerHTML =
                    `<img class="chatlog__reference-avatar" src="${author.avatarURL() ?? defaultPFP}" alt="Avatar" loading="lazy">
            ...
            // message author pfp
            const author = message.author ?? DummyUser;
            ...
            authorAvatar.src = author.avatarURL() ?? defaultPFP;
            ...
            if (message.attachments && message.attachments.size > 0) {
                for (const attachment of message.attachments.values()) {
                    ...
                    if (['png', 'jpg', 'jpeg', 'gif'].includes(attachmentType)) {
                        ...
                        attachmentVideo.setAttribute('alt', 'Video attachment');
                        ...
                        attachmentAudio.setAttribute('alt', 'Audio attachment');
        ...
        const fileTrans = new AttachmentBuilder(Buffer.from(dom.serialize()), { name: `${interaction.channel.id}.html` });
        channel.send({ files: [fileTrans] });
    }

    function formatContent(content: any, allowExtra = false, replyStyle = false, purify = he.escape) {

...
import FunctionModel from '../../database/models/functions';
...
module.exports = (client: any) => {
    ...
    client.errNormal = async function ({
        ...
        console.log(embed, error, type, content, components)
    ...
    client.embed = async function ({
        ...
        const functiondata = await FunctionModel.findOne({ Guild: interaction.guild.id })
    ...
    client.simpleEmbed = async function ({
        ...
        if (thumbnail) embed.setThumbnail(thumbnail);
        if (fields) embed.addFields(fields);
        if (author) embed.setAuthor(author[0]);
        if (url) embed.setURL(url);
        if (color) embed.setColor(color);
        if (functiondata && functiondata.Color && !color) embed.setColor(functiondata.Color as Discord.ColorResolvable)
